...
const ABOUT_STORAGE_KEY = "boysschool-about-seen";
const MAX_MESSAGES_PER_CONVERSATION = 42;
const MAX_MESSAGES_PER_WINDOW = 3;
const RATE_WINDOW_MS = 8000;

// NEXT_PUBLIC_* variables are inlined at build time, so this is a build-time constant.
// Evaluating it here (outside the component) means the initial state is correct on the
// very first render and avoids a flash from "loading" → "error" when the key is absent.
const apiKeyMissing = !process.env.NEXT_PUBLIC_GROQ_API_KEY;

export function ChatWindow() {
  const [appState, setAppState] = useState<AppState>(
    apiKeyMissing ? "error" : "loading",
  );
    ...
    >
      ...
      <InputBar
        ...
        placeholder={
          workerStatus === "loading"
            ? "Loading…"
            : "Ask about school policies, procedures, curriculum…"
        }
      />
    </main>
  );
