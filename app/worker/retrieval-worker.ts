...
async function _init() {
  // 1. Load pre-built boysschool-chunks.json (primary path)
  try {
    const response = await fetch(`${basePath}/data/boysschool-chunks.json`);
    if (response.ok) {
  ...
  if (chunks.length === 0) {
    try {
      const response = await fetch(
        `${basePath}/data/BoysSchool_StaffOnboarding.md`,
      );
